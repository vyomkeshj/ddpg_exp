    ...
    def move_to_replay(self, final_pos): # todo: compute the change to the her here
        ...
        for istate, iaction, inext_state, ireward, idone in zip(state, action, next_state, reward, done):
            ...
            #
             ...
             #istate[4] = istate[4] + self.current_final[2] - final_pos[2]      #todo: for difference in observation
            #
             ...
             #istate[2] =  final_pos[0]        # state is targetpos - robotpos, subtract by targetpos (final before change) and
             #istate[3] =  final_pos[1]
             #istate[4] =  final_pos[2]      #todo: for difference in observation#

             #inext_state[2] = final_pos[0]
             #inext_state[3] = final_pos[1]
             #inext_state[4] = final_pos[2]


             self.replay_storage.add(istate.cpu(), iaction.cpu(), inext_state.cpu(), ireward.cpu(), idone.cpu())        #modify before
             self.temp_storage.flush()   #clear out the temp storage for the next epoch
